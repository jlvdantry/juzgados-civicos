...
class RegisterController extends Controller
{
    ...
    */
     ...
     * @return \App\User
     */
    protected function create_alcaldia(array $data)
    {
        return User::create([
            'nombres' => $data['nombres'],
            'ape_pat' => $data['ape_pat'],
    ...
    {
        log::debug('entro RegisterController.php');
        $this->validator($request->all())->validate();
        log::debug('entro RegisterController.php paso validator');
        log::debug($request['rfc']);

        if ($request->has('rfc')) {
            $dato=User::where('rfc','=',$request['rfc']);
            if ($dato->count()>0) {
                log::debug('Auth/RegisterController.php entro en if');
                return response()->json(['errors' => ['rfc' => 'el RFC ya esta registrado']], 401);
            }
        }
